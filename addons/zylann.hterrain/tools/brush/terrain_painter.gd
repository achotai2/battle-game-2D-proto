...
signal slope_limit_changed
signal height_limit_changed
...
var _height_limit_min := -10000.0
var _height_limit_max := 10000.0

# [mode][index] => { slope_low, slope_high, height_min, height_max }
var _brush_params := {}
...
func set_detail_index(di: int):
	...
	if _mode == MODE_DETAIL:
		_load_brush_params(MODE_DETAIL, di)
...
func set_texture_index(i: int):
	...
	_texture_index = i
	if _mode == MODE_SPLAT:
		_load_brush_params(MODE_SPLAT, i)
...
func set_slope_limit_angles(low: float, high: float):
	if _slope_limit_low_angle == low and _slope_limit_high_angle == high:
		return
	_slope_limit_low_angle = low
	_slope_limit_high_angle = high
	slope_limit_changed.emit()
	_save_brush_params(_mode, _get_current_index())


func get_height_limit_min() -> float:
	return _height_limit_min


func get_height_limit_max() -> float:
	return _height_limit_max


func set_height_limits(min_h: float, max_h: float):
	if _height_limit_min == min_h and _height_limit_max == max_h:
		return
	_height_limit_min = min_h
	_height_limit_max = max_h
	height_limit_changed.emit()
	_save_brush_params(_mode, _get_current_index())
...
func _save_brush_params(mode: int, index: int):
	if not _brush_params.has(mode):
		_brush_params[mode] = {}
	_brush_params[mode][index] = {
		"slope_low": _slope_limit_low_angle,
		"slope_high": _slope_limit_high_angle,
		"height_min": _height_limit_min,
		"height_max": _height_limit_max
	}


func _load_brush_params(mode: int, index: int):
	var params = null
	if _brush_params.has(mode):
		if _brush_params[mode].has(index):
			params = _brush_params[mode][index]

	if params != null:
		_slope_limit_low_angle = params.slope_low
		_slope_limit_high_angle = params.slope_high
		_height_limit_min = params.height_min
		_height_limit_max = params.height_max
	else:
		# Defaults
		_slope_limit_low_angle = 0.0
		_slope_limit_high_angle = PI / 2.0
		_height_limit_min = -10000.0
		_height_limit_max = 10000.0
...
func set_mode(mode: int):
	...
	if _mode == mode:
		return
	_mode = mode
	_load_brush_params(_mode, _get_current_index())
...
func _paint_splat4(data: HTerrainData, position: Vector2):
	...
	_set_height_limit_shader_params(p)
	p.set_image(image, texture)
	p.paint_input(position)
...
func _set_height_limit_shader_params(p: HT_Painter):
	p.set_brush_shader_param("u_height_min", _height_limit_min)
	p.set_brush_shader_param("u_height_max", _height_limit_max)
