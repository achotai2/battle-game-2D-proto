...
@onready var _height_limit_label : Label = $GridContainer/HeightLimitLabel
@onready var _height_limit_control : Control = $GridContainer/HeightLimitControl
@onready var _height_min_spinbox : SpinBox = $GridContainer/HeightLimitControl/MinHeight
@onready var _height_max_spinbox : SpinBox = $GridContainer/HeightLimitControl/MaxHeight
...
func _ready():
	...
	_height_min_spinbox.value_changed.connect(_on_height_limit_changed)
	_height_max_spinbox.value_changed.connect(_on_height_limit_changed)
...
func set_terrain_painter(terrain_painter: HT_TerrainPainter):
	if _terrain_painter != null:
		...
		_terrain_painter.slope_limit_changed.disconnect(_on_painter_slope_limit_changed)
		_terrain_painter.height_limit_changed.disconnect(_on_painter_height_limit_changed)
	...
	if _terrain_painter != null:
		...
		_on_painter_slope_limit_changed()
		_on_painter_height_limit_changed()
		...
		_terrain_painter.slope_limit_changed.connect(_on_painter_slope_limit_changed)
		_terrain_painter.height_limit_changed.connect(_on_painter_height_limit_changed)
...
func set_display_mode(mode: int):
	...
	var show_slope_limit := \
		mode == HT_TerrainPainter.MODE_SPLAT or \
		mode == HT_TerrainPainter.MODE_DETAIL or \
		mode == HT_TerrainPainter.MODE_COLOR
	var show_height_limit := show_slope_limit
	...
	_set_visibility_of(_height_limit_label, show_height_limit)
	_set_visibility_of(_height_limit_control, show_height_limit)
...
func _on_height_limit_changed(_ignored=0):
	var min_h = _height_min_spinbox.value
	var max_h = _height_max_spinbox.value
	_terrain_painter.set_height_limits(min_h, max_h)


func _on_painter_slope_limit_changed():
...
func _on_painter_height_limit_changed():
	var min_h = _terrain_painter.get_height_limit_min()
	var max_h = _terrain_painter.get_height_limit_max()
	_height_min_spinbox.value = min_h
	_height_max_spinbox.value = max_h
