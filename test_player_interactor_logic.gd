...
class MockSensor extends Area2D:
	var _overlaps: Array[Area2D] = []
	func set_overlaps(overlaps: Array[Area2D]):
		_overlaps = overlaps
...
class MockInteractable extends Interactable:
	func _init(p_priority: int, p_pos: Vector2):
		my_priority = p_priority
		priority = p_priority # Using Area2D priority just in case
		position = p_pos
...
func test_initial_sync():
	...
	var i1 = MockInteractable.new(10, Vector2(0,0))
...
func test_signal_updates():
	...
	var i1 = MockInteractable.new(10, Vector2(100, 100))
